...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/07/15 21:10:58 by dhawkgir          #+#    #+#             */
...
void	texture_setter(t_data *data)
{
	...
	else if (data->side == 1 && 0 > data->step_y && data->text_number != 3)
	{
		data->loaded_texture = data->texture_3;
		data->text_number = 3;
	}
	else if (data->side == 1 && 0 <= data->step_y && data->text_number != 4)
	{
...
data->line_height) / 256;
			...
			while (data->i < 3)
			{
				data->img->address[data->y * data->img->line_length + data->x * \
data->img->bits_per_pixel / 8 + data->i] = \
data->loaded_texture->address[data->tex_y * \
